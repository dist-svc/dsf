...
pub trait Backend {
    ...
    fn with<R>(&self, f: impl FnMut(&mut Self::Conn) -> Result<R, StoreError>) -> Result<R, StoreError>;
}
...
impl Backend for Pool<ConnectionManager<SqliteConnection>> {
    ...
    fn with<R>(&self, mut f: impl FnMut(&mut Self::Conn) -> Result<R, StoreError>) -> Result<R, StoreError> {
        let mut c = self.get()
            .map_err(|_| StoreError::Acquire)?;
        f(&mut c)
    }
}
...
mod tests {
    ...
    fn bench_db_write_mem(b: &mut Bencher) {
        let mut store = Store::new_rc("sqlite://:memory:?cache=shared", Default::default())
    ...
    fn bench_db_read_mem(b: &mut Bencher) {
        ...
        let data: Vec<_> = (0..100).map(|_| {
            svc.publish_data_buff::<Vec<u8>>(Default::default()).unwrap().1
        }).collect();
        ...
        b.iter(|| {
            ...
            store.load_object(&svc.id(), &d.signature(), &svc.keys()).unwrap();
        })
    }
    ...
    fn bench_db_write_file_wal(b: &mut Bencher) {
        ...
        let mut store = Store::new_rc(&format!("{d}/sqlite-wal.db"), Default::default())
            .expect("Error opening store");
